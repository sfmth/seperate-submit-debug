...
async def test_start(dut):
    ...
    await with_timeout(RisingEdge(dut.uut.mprj.projtes1.wrapped_ibnalhaytham_1.active), 3000, 'us')
...
async def test_all(dut):
    ...
    dut.uut.mprj.projtes1.io_in.value = 0 # initialize

    cocotb.fork(clock.start())

    await Timer(10, units="us")
    
    dut.uut.mprj.projtes1.io_in[11].value = 1 # set clock input

    dut.uut.mprj.projtes1.io_in[12].value = 0

    dut.uut.mprj.projtes1.io_in[8].value = 1 # set inst_mem mode
    dut.uut.mprj.projtes1.io_in[9].value = 1
    await Timer(10, units="us")

    await with_timeout(RisingEdge(dut.uut.mprj.projtes1.wrapped_ibnalhaytham_1.core1.processor_1.clk), 500, 'us')


    # wait for the reset signal - time out if necessary - should happen around 165us
    await with_timeout(RisingEdge(dut.uut.mprj.projtes1.wrapped_ibnalhaytham_1.core1.processor_1.reset), 500, 'us')
    await FallingEdge(dut.uut.mprj.projtes1.wrapped_ibnalhaytham_1.core1.processor_1.reset)
